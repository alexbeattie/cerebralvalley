"""Core data models for the causality review.

Design principle carried over from variant_curator: every claim carries the
source it came from, and the tool separates what it can support from what it
cannot. A causality score a clinician can act on has to show its work -- which
of the patient's features each variant explains, and which it leaves on the
table.
"""
...
class ReportedVariant:
    ...
    This is an INPUT to step 2, not something we re-derive. We take the lab's
    call as given and ask a different question: does it fit THIS patient?
    """
