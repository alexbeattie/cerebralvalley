...
This tests the ranking + discrimination logic against live HPO data. It is a
...
Metrics:
  - Top-1 accuracy: fraction of cases where the true gene ranks first.
  - Top-3 accuracy: fraction where the true gene is in the top 3.
  - MRR: mean reciprocal rank of the true gene.
...
# Curated solved cases. Every HPO term below was confirmed present (exactly or
# via an is_a ancestor) in its gene's HPO annotation set.
CASES: list[EvalCase] = [
    ...
    EvalCase("Malignant hyperthermia", "RYR1", [
        ...
        Phenotype("HP:0003236", "Elevated creatine kinase"),
...
def run() -> dict:
    ...
    with get_client() as client:
        for case in CASES:
            ...
            rows.append((case.name, case.true_gene, rank, winner, report.fits[0].score if report.fits else 0.0))
...
def _print(rows, summary) -> None:
    print("=" * 72)
    print(f"  Causality-ranking eval — {summary['n']} solved cases vs a {summary['panel_size']}-gene panel")
    print("=" * 72)
    print(f"\n  {'case':32s} {'true':7s} {'rank':4s} {'top gene':8s}")
    for name, gene, rank, winner, _score in rows:
